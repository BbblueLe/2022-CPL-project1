...
{
    int space[4][4];
} block[12][4]; // 用于存储12种基本形状方块的各自的4种形态的信息，共40种
...
void InitBlockInfo()
{
    ...
    for (int i = 0; i <= 2; i++)
    {
        block[7][0].space[i][1] = 1;
    }
    // “三方块 l”形
    for (int i = 1; i <= 2; i++)
        block[8][0].space[i][1] = 1;
    block[8][0].space[2][2] = 1;

    // “五方块L ”形(瘦长)
    for (int i = 0; i <= 3; i++)
        block[9][0].space[i][1] = 1;
    block[9][0].space[3][2] = 1;

    // “五方块L型”(扁平)
    for (int i = 0; i <= 3; i++)
        block[10][0].space[1][i] = 1;
    block[10][0].space[2][3] = 1;

    // “五方块L型”(均匀)
    for (int i = 0; i <= 2; i++)
        block[11][0].space[i][1] = 1;
    for (int i = 2; i <= 3; i++)
        block[11][0].space[2][i] = 1;
...
void color(int c)
{
    ...
    //SetConsoleTextAttribute是一个API（应用程序编程接口）
}
// 画出方块
void DrawBlock(int shape, int form, int x, int y)
{
...
void ReadGrade()
{
    FILE *pf = fopen("highest_score.txt", "r"); // 以只读方式打开文件
    if (pf == NULL)                             // 打开文件失败
    {
        pf = fopen("highest_score.txt", "w"); // 以只写方式打开文件（文件不存在可以自动创建该文件）
...
void WriteGrade()
{
    FILE *pf = fopen("highest_score.txt", "w"); // 以只写方式打开文件
    if (pf == NULL)                             // 打开文件失败
    {
        printf("保存最高得分记录失败\n");
        exit(0);
